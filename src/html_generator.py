...
def generate_html_from_json(json_file_path: str, template_dir: str, template_name: str, output_dir: str):
    ...
    try:
        ...
        page_title = f"ArXiv CS.CV Papers (Autonomous Driving, VLA, 3D Understanding, Robotics) - {report_date.strftime('%B %d, %Y')}"
    except (IndexError, ValueError):
        logging.warning(f"Could not extract date from filename {filename}. Using default.")
        today = date.today()
        formatted_date = today.strftime("%Y_%m_%d")
        page_title = f"ArXiv CS.CV Papers (Autonomous Driving, VLA, 3D Understanding, Robotics) - {today.strftime('%B %d, %Y')}"
